...
import { GetPageContentResponse } from "lib/notion/types";
...
function formatProperties(properties: PageObjectResponse["properties"]) {
  ...
  const formatterByType: Record<string, (key: string, value: any) => void> = {
    ...
    [PropertyKeys.relation]: (key: string, value: any) => {
      const { relation } = (value as any)[value.type][0];
      return { [key]: relation };
    },
...
function formatBlocks(mdBlocks: GetPageContentResponse["mdBlocks"]): { markdown: string } {
  ...
  return n2m.toMarkdownString(mdBlocks, "markdown", 0) as { markdown: string };
}
